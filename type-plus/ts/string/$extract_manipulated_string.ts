...
export type $ExtractManipulatedString<T extends string> =
	([T, unknown] extends [unknown, T]
		? T
		: $ExtractManipulatedString._UpperOrElse<T,
			$ExtractManipulatedString._LowerOrElse<T,
				$ExtractManipulatedString._CapOrElse<T,
					$ExtractManipulatedString._UncapOrElse<T, T>>>>
	)
