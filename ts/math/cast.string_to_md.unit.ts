...
it('casts negative number to ["number", "-", Significand, -Exponent]', () => {
	...
	testType.equal<StringToMathDevice<'-0.123'>, ['number', '-', [0, 1, 2, 3], 1]>(true)
	testType.equal<StringToMathDevice<'-123.45'>, ['number', '-', [1, 2, 3, 4, 5], 3]>(true)
})
...
it('casts positive number to ["number", "+", Significand, -Exponent]', () => {
	...
	testType.equal<StringToMathDevice<'0.123'>, ['number', '+', [0, 1, 2, 3], 1]>(true)
	testType.equal<StringToMathDevice<'0.000123'>, ['number', '+', [0, 0, 0, 0, 1, 2, 3], 1]>(true)
	testType.equal<StringToMathDevice<'123.45'>, ['number', '+', [1, 2, 3, 4, 5], 3]>(true)
})
