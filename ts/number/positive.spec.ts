...
it('returns never if T is number as it can contain float', () => {
	type.never<Positive<number>>(true)
})
...
it('returns T if T is union of mixing positive and negative value', () => {
	// could not find a way to avoid the distribution.
	// It actualy returning `Then | Else`,
	// but since `Else = never`, that resolves to `Then`.
	type.equal<Positive<1 | -1>, 1 | -1>(true)
})
...
it('can override Then/Else', () => {
	...
	type.equal<Positive<any, 1, 2>, 2>(true)
