import { UnionOfValues, assertType, IsEqual, isType } from '../index.js'

test('get value of generic array', () => {
	type Actual = UnionOfValues<string[]>
	assertType.isTrue(true as IsEqual<string, Actual>)
})

test('tuple get union type of values', () => {
	type Actual = UnionOfValues<[string, boolean]>
	assertType.isTrue(true as IsEqual<string | boolean, Actual>)
})
